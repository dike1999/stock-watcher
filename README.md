...
│   └── delete.png       # 删除按钮图标
└── pages/               # 页面组件
    ├── FloatWindow.css  # 浮窗样式
    └── FloatWindow.tsx  # 浮窗组件
...
| `src/pages/FloatWindow.tsx` | 浮窗 React 组件，股票管理界面 |
| `src/App.tsx` | React 应用入口，路由和全局布局 |
...
浮窗面板使用 React 组件实现，位于 `src/pages/FloatWindow.tsx`，包含：
...
| `src/index.css` | 全局样式 | ~50 行 |
| `src/pages/FloatWindow.tsx` | 浮窗 React 组件 | ~150 行 |
| `src/pages/FloatWindow.css` | 浮窗样式 | ~100 行 |
| `src/assets/close.png` | 关闭按钮图标 | - |
| `src/assets/delete.png` | 删除按钮图标 | - |
...
| react | UI 框架 | 浮窗使用 React 组件方案 |
| react-dom | React DOM 渲染 | 用于渲染 React 组件 |
| react-router-dom | 路由管理 | 用于页面路由 |
